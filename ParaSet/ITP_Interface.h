...
{
	char stoppoint;	//1表示inpos,2表示stoptime,3表示followb										//译码实现
...
{
//加速度
	double acc_acc;//加速度				单位：radian/周期*周期	(用于关节加减速和旋转加减速)
	double dec_acc;//减加速度
	...
	double worldAcc_acc;//加速度		//单位：mm/周期*周期
...
typedef struct ITP_InputData	//输入
{
	...
	*************************************************************/
	...
	double jogMoveVal;	//0表示手动速度模式，非0表示手动位置模式，且表示移动距离
	//当模式为ITP_Mode_JointOp_SpeedAndIncMode时以下触发信号有效
	//单位为：弧度/插补周期或mm/插补周期
	...
	//当模式为ITP_Mode_CoordinateOp_SpeedAndIncMode时以下触发信号有效
	double moveSpeed;
...
//插补预处理报警(仅针对自动运行模式)
	int	progNo2;	//预处理异常报警程序号(插补只输出报警，不做处理)
