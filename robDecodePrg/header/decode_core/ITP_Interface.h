...
#define ITP_Key_InputShareMem	123
...
/****************************************************************
插补模块分2类操作：
1、关节操作(速度控制和给定角度控制)	ITP_Mode_JointOp
2、坐标操作	ITP_Mode_CoordinateOp
	a、速度控制
	b、位置控制（坐标系）（定时加减速）
****************************************************************/
#define ITP_Mode_JointOp_SpeedMode	2	//速度运行
#define ITP_Mode_JointOp_PosMode	3	//通过给定的位置，关节单独加减速

#define ITP_Mode_CoordinateOp_SpeedMode	12	//速度运行
#define ITP_Mode_CoordinateOp_PosMode	13	//译码运行
...
(可选、模态指令)ITP_SpeedData speed;	//速度
...
#define ITP_CMD_MoveExtJ		(ITP_BgnNum_Move+1)
...
//#define ITP_CMD_StepBwdPath		(ITP_BgnNum_Move+30)


/*****************************************************************
异常类型(插补运动指令)
*****************************************************************/
#define ITP_Error_No	0	//无异常
#define ITP_Error_Outside	1//超出关节范围
#define ITP_Error_Sing	2	//奇异点
...
typedef struct ITP_WobjData		//用户坐标系
{
	...
	char *ufmec;	//机械单元名称，当ufprog==0时有效
...
{
	char stoppoint;	//1表示inpos,2表示stoptime,3表示followb
	char progsynch;	//0表示该程序段未执行完可进行下一程序段的执行，1相反
	ITP_Inpos inpos;	//停止精度数据
	double stoptime;	//停止时间
	double followtime;	//跟随时间（使用传送装置）
...
{
	int flagAxisType[ITP_ExtaxAxisNum];//表示该外轴是否需要使用及类型,0表示没有使用，1表示旋转关节，2表示直线运动
...
//奇异点
	char sing_wrist;//工具方向发生偏移以避开奇异点
	char sing_lockaxis4;//当5轴接近0度时，将4轴锁定在0或+-180。
	char sing_base;	//不处理	
//机器人配置
	char conf_jsup;	//关节指令时，进行关节配置监视
	char conf_lsup;	//直线圆弧指令时，进行关节配置监视

	char acc_way;	//加减速方法：0表示S曲线，1表示直线

	//double conf_ax1;	//1轴的最大允许偏移
	//double conf_ax4;	//4轴的最大允许偏移
	//double conf_ax6;	//6轴的最大允许偏移

	//double pathresol;
	//char motionsup;
	//double tunevalue;

	char worldacclim;	//是否进行世界坐标系最大加速度限制
	double worldaccmax;	//最大世界坐标系加速度

//最大加减速度——针对关节
	char acclim;	//是否进行最大轨迹加速度限制
	double accmax;	//最大轨迹加速度

	char decellim;	//是否进行最大轨迹减速度限制
	double decelmax;	//最大轨迹减速度

//圆弧指令工具方向
	char cirpathreori;//0标准的插补，1工件框架插补，2工具方向指向圆心
...
typedef struct ITP_ParaData //参数标识
{
	int type;	//参数类型
	int flagPos;	//该类型参数位置号
}ITP_ParaData;
...
typedef struct ITP_CmdData	//插补内部数据结构，整合读取的指令数据，用于加减速和插补
{
	...
	****************************************************/
	struct ITP_ParaData P;//位姿或关节坐标
	struct ITP_ParaData V;//速度
	struct ITP_ParaData Z;//停止方式
	struct ITP_ParaData T;//工具坐标系
	struct ITP_ParaData W;//用户坐标系
...
typedef struct ITP_InputData	//输入
{
	...
	#define ITP_Mode_CoordinateOp_SpeedMode	12	//速度运行
	#define ITP_Mode_CoordinateOp_PosMode	13	//给定角度运行
	*************************************************************/
	char mode;//模式

	ITP_Pose toolPose;	//手动模式下的工件坐标系（基坐标：法兰盘坐标系）

	//当模式为ITP_Mode_JointOp_SpeedMode，ITP_Mode_CoordinateOp_SpeedMode时以下触发信号有效
	//单位为：弧度/插补周期或mm/插补周期
	...
	//当模式为ITP_Mode_CoordinateOp_SpeedMode时以下触发信号有效
	...
	ITP_RobTarget tarRob;	//直接手动运动到指定位置
...
typedef struct ITP_OutputData	//输出
{
	...
	int alarmNo;	//输出报警号 0表示无报警	1表示超程报警	2表示奇异点报警
	//int intno;			//中断号
	//int rob_id;			//机械单元ID


	int progNo_exe;			//执行程序号
	int progNo_read;		//已读取程序号

	ITP_RobTarget probePoint;//探针功能记录点

	ITP_RobTarget curRob;			//当前位姿(基坐标系)
	ITP_RobTarget curRobUserCoor;	//当前位姿(用户自定义坐标系),仅当用户选取了用户坐标系有效

	ITP_JointTarget curJoint;//当前关节坐标
