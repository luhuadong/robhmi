    ...
    while(1)
    {
        ...
        if(joggingPar.motionMode == Jogging::PAR_AXIS1_6)   // ---------------- 关节操作 ---------------- （1、2、3、4、5、6）
        {
            for(int i=0; i<POSITION_ROWS-1; i++) {
                if(positionFmt.angleUnit == Jogging::PAR_SHOW_DEGREES)  // 关节操作 & 单位度
                {   poseData[i] = QString::number(ITP_radian2angle(pOutputData->curJointTarget.robax.rax[i]), 'f', 2);}
                else if(positionFmt.angleUnit == Jogging::PAR_SHOW_RADIANS) // 关节操作 & 单位弧度
        ...
        else    // -------------------- 坐标操作 --------------------
        {
            ...
            if(positionFmt.orientationFmt == Jogging::PAR_SHOW_QUATERNION)  // 坐标操作 & 四元数 （X、Y、Z、q1、q2、q3、q4）
            {
            ...
            else if(positionFmt.orientationFmt == Jogging::PAR_SHOW_EULERANGLES)    // 坐标操作 & 欧拉角 （X、Y、Z、EZ、EY、EX）
            {
                /* ************************************************************************
                ...
                if(positionFmt.angleUnit == Jogging::PAR_SHOW_DEGREES)  // 坐标操作 & 欧拉角 & 单位度
                {
                ...
                else if(positionFmt.angleUnit == Jogging::PAR_SHOW_RADIANS) // 坐标操作 & 欧拉角 & 单位弧度
                {
